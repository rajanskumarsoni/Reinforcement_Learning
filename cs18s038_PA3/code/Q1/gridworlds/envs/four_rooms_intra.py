...
class FourRoomsIntra(gym.Env):
	...
	def room_options(self, option, room, coord, in_hallway) :
		"""It might happen option  go in some other room  instead of goal so there we should also stop it"""
		...
		if option == 4 :
			...
			if coord == [4,1] :
				...
				action_took = 2
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# return [3, coord], True, action_took
			elif coord[1] == 1 :
				action_took = 2
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1
			if new_state == [3, self.hallway_coords[3]] or new_state == [0, self.hallway_coords[0]] or new_state==[1,[2,0]]:
				return new_state, reward, True, action_took
			else:
				return new_state, reward, False, action_took
			...
			if room == 3 :
				...
				action_took = 1
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# return [0, self.hallway_coords[0].copy()], True, action_took
			elif coord[0] == 2 :
				action_took = 1
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1
			if new_state == [0, self.hallway_coords[0]] or [3, self.hallway_coords[3]] or new_state == [3,[-1,1]] :
				return new_state, reward, True, action_took
			else:
				return new_state, reward, False, action_took

			# return [0, coord ], False, action_took
		...
		# options for room 1
			...
			if coord == [2,0] :
				...
				# return [0, coord], True, action_took
			elif coord[0] == 2 :
				action_took =3
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] -=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1
			if [0, self.hallway_coords[0]] == new_state  or new_state == [1, self.hallway_coords[1]] or new_state == [2,[0,2]] :
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took
			# return [1, coord], False, action_took
			...
			if room == 0 :
				...
				action_took = 2
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# return [1, self.hallway_coords[1].copy()], True, action_took
			elif coord[1] == 2 :
				action_took = 2
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1
			if new_state == [1, self.hallway_coords[1]] or new_state ==  [0, self.hallway_coords[0]]  or new_state ==[0,[2,4]] :
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took
		...
		# options for room 2
			...
			if coord == [0,2] :
				...
				# return [1, coord], True, action_took
			elif coord[1] == 2 :
				action_took = 0
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] -=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1
			if [1, self.hallway_coords[1]] == new_state  or [ 2, self.hallway_coords[2]] == new_state or [3,[3,4]] == new_state:
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took

			# return [ 2, coord], False, action_took
			...
			if room == 1 :
				...
				action_took = 3
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# return [ 2, self.hallway_coords[2]], True, action_took
			elif coord[0] == 2 :
				action_took= 3
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] -=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1
			if [ 2, self.hallway_coords[2]] == new_state or [1, self.hallway_coords[1]] == new_state or [1,[5,2]]:
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took
		...
		# options for room 3
			...
			if coord == [3,4] :
				...
				# return [2, coord], True, action_took
			elif coord[0] == 3 :
				action_took = 1
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1 
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[0] +=1
			if [2, self.hallway_coords[2]] == new_state or [3, self.hallway_coords[3]] == new_state or [0,[4,1]] :
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took

			# return [ 3, coord], False, action_took
		elif option == 11:
			# print("hgghii")
			if room == 2 :
				action_took = 3
				...
				new_state, reward = self.room_actions( action_took, room, coord, in_hallway)
				# coord[1] +=1
			if [ 3, self.hallway_coords[3]] == new_state or [ 2, self.hallway_coords[2]] == new_state or new_state == [2,[2,0]] :
				return new_state, reward, True, action_took
			else :
				return new_state, reward, False, action_took
	...
	def room_actions(self, action, room, coord, in_hallway) :
		...
		if in_hallway: # hallway option
			...
			# else :
			# 	[room2, coord2], status, option = self.room_options(option,room, coord, in_hallway)
			...
			if action == hallway_info[0]:
				...
				coord2 = [row, col]
			# else :
			# 	[room2, coord2], status , option= self.room_options(option,room, coord, in_hallway = False)
	...
	def step(self, option):
		...
		if self.state == self.terminal_state:
			...
			self.done = True
			return self.state, self.get_reward(), self.done, {0:op_status, 1: action_took}
		...
		# if self.done == False :
		if option >3:
			# new_state, reward, False, action_took
			[room2, coord2], reward, op_status, action_took= self.room_options(option,room, coord, in_hallway)
		else :
			[room2, coord2], reward  = self.room_actions( option, room, coord, in_hallway)
		...
		self.state = new_state
		return new_state, reward, self.done, {0:op_status, 1: action_took}
